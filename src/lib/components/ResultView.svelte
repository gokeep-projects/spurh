...
<style>
  ...
  .ai-answer { position: relative; display: flex; flex-direction: column; width: 100%; margin: 0 auto; max-height: min(32vh, 300px); overflow: hidden; border-radius: 16px; background: linear-gradient(160deg, color-mix(in srgb, var(--c-violet) 9%, var(--panel)), var(--panel) 55%, color-mix(in srgb, var(--c-cyan) 6%, var(--panel))); box-shadow: 0 14px 44px color-mix(in srgb, var(--c-violet) 12%, transparent), inset 0 1px 0 color-mix(in srgb, #fff 8%, transparent); animation: aiAnswerIn .4s cubic-bezier(.2,.9,.3,1.2); --ai-angle: 0deg; }
  ...
  @keyframes aiAnswerIn { from { opacity: 0; transform: translateY(8px) scale(.985); } to { opacity: 1; transform: translateY(0) scale(1); } }
  .ai-answer .ai-answer-body { scrollbar-width: thin; scrollbar-color: color-mix(in srgb, var(--c-violet) 45%, var(--line)) transparent; }
