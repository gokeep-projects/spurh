  ...
  <div class="net-head">
    ...
    <div class="net-tools">
      ...
      {:else if tab === 'tcp'}
        ...
        {/if}
        <label class="net-field grow net-data"><span>数据</span><textarea rows={tcpHexMode ? 8 : 13} value={tcpData} oninput={(e) => (tcpData = e.currentTarget.value)} placeholder="要发送的内容，如 GET / HTTP/1.1" spellcheck="false" onkeydown={(e) => e.ctrlKey && e.key === 'Enter' && sendTcp()}></textarea><small class="net-data-hint">Ctrl+Enter 发送 · 支持多行报文 · 可拖拽调整高度</small></label>
