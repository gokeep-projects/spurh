...
<div class="log-panel">
  <header class="log-bar">
    <div class="log-bar-actions">
      ...
      <button class="log-btn" onclick={() => { text = ''; analysis = null; }}><span>✕</span>清空</button>
...
<style>
  ...
  .log-input small { color: var(--muted-2); font-size: 10px; }

  .log-error { padding: 12px; color: var(--danger); font-size: 11px; border: 1px solid color-mix(in srgb, var(--danger) 30%, var(--line)); border-radius: 10px; background: color-mix(in srgb, var(--danger) 5%, transparent); }
  .log-loading { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 40px 20px; color: var(--muted); }
  .log-loading p { margin: 0; font-size: 11px; }
