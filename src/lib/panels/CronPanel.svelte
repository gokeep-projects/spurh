<script lang="ts">
  ...
  const EXAMPLES = [
    ...
    { label: '每月 1 日 00:00（月报）', expr: '0 0 1 * *' },
    { label: '每月 15 日 09:00', expr: '0 9 15 * *' },
    { label: '工作日每 30 分钟（9-18 点）', expr: '*/30 9-18 * * 1-5' },
    { label: '每年 1 月 1 日 00:00', expr: '0 0 1 1 *' },
    { label: '每周一至周五 18:00（收尾）', expr: '0 18 * * 1-5' },
    ...
    { label: '每分钟心跳', expr: '* * * * *' },
    { label: '每半小时心跳', expr: '*/30 * * * *' },
    { label: '每小时清理临时文件', expr: '0 * * * *' },
    { label: '每天 9-18 点每小时', expr: '0 9-18 * * *' },
    { label: '每天 4 次 09/13/17/21 点', expr: '0 9,13,17,21 * * *' },
    { label: '工作日每 10 分钟', expr: '*/10 * * * 1-5' },
    { label: '工作日 08:30 打卡', expr: '30 8 * * 1-5' },
    { label: '每月 15 号发薪日 09:00', expr: '0 9 15 * *' },
    { label: '每月最后一天 12:00 统计', expr: '0 12 L * *' },
    { label: '每周日 10:00 备份', expr: '0 10 * * 0' },
    { label: '每年 6 月 30 日 18:00 年度结算', expr: '0 18 30 6 *' },
    { label: '每 8 分钟', expr: '*/8 * * * *' },
    { label: '工作日 12:00（午间）', expr: '0 12 * * 1-5' },
    { label: '每月第 2 个周一 09:00', expr: '0 9 * * 1#2' },
    { label: '每月最后一个周五 09:00', expr: '0 9 * * 5L' },
    { label: '每 40 分钟', expr: '*/40 * * * *' },
    { label: '每 3.5 小时（03:30/07:00 等）', expr: '30 */3 * * *' },
    { label: '每天 05:30（早起任务）', expr: '30 5 * * *' },
    { label: '工作日 13:00（午休后提醒）', expr: '0 13 * * 1-5' },
    { label: '每周五 16:30（周报提醒）', expr: '30 16 * * 5' },
    { label: '每季度首日 02:00（季报）', expr: '0 2 1 1,4,7,10 *' },
    { label: '每月 1 日和 16 日 03:00（双月备份）', expr: '0 3 1,16 * *' },
  ];
...
<div class="cron-panel">
  <p class="cron-intro">Cron 表达式用于<code>定时任务</code>调度：按固定间隔或指定时间触发脚本、备份、爬虫等。选好条件或点右侧常用表达式，下方实时生成表达式与未来 5 次执行时间。</p>
