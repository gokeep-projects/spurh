...
<style>
  ...
  .crypto-group button { height: 26px; padding: 0 10.5px; cursor: pointer; color: var(--muted); font-size: 12px; border: 0; border-radius: 6px; background: transparent; white-space: nowrap; transition: all .15s ease; }
  ...
  .crypto-key-row label > span { color: var(--muted); font-size: 13px; }
  .secret-wrap { position: relative; display: flex; align-items: center; }
  .secret-wrap input { height: 32px; width: min(26vw, 260px); padding: 0 34px 0 10px; color: var(--text); font: 500 13px 'Cascadia Code', monospace; border: 1px solid var(--line); border-radius: 6px; outline: 0; background: var(--bg); }
  .secret-wrap input:focus { border-color: color-mix(in srgb, var(--accent) 50%, var(--line)); box-shadow: 0 0 0 3px var(--accent-soft); }
  .secret-toggle { position: absolute; right: 4px; width: 26px; height: 26px; display: grid; place-items: center; cursor: pointer; font-size: 13px; border: 0; border-radius: 5px; background: transparent; }
  /* svelte-ignore css_unused_selector */
  .secret-toggle :global(svg) { width: 14px; height: 14px; }
  .secret-toggle:hover { background: var(--hover); }
  .crypto-clear { height: 30px; padding: 0 10px; cursor: pointer; color: var(--muted); font-size: 12px; border: 1px solid transparent; border-radius: 6px; background: transparent; }
  .crypto-clear:hover { color: var(--text); border-color: var(--line); }
  .crypto-note { color: var(--muted-2); font-size: 13px; }
